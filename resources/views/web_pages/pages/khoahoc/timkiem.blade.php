        ...
        <div class="row" style="margin-top: 50px; ">
            ...
            <div class="col-sm-9 padding-right">
                <div class="features_items"><!--items-->
                    @if(count($tk_khoahoc)>0)
                        <h2 class="title text-center">Danh sách các khóa học có từ : "{{$tukhoa}}"</h2>
                        @foreach($tk_khoahoc as $kh)
                        ...
                        @endforeach
                    @else
                        <h2 class="title text-center">Không có khóa học có từ : "{{$tukhoa}}"</h2>
                    @endif
                </div><!--features_items-->
                <div class="pagination-area" style="float: right">
                    <ul class="pagination pagination-sm m-t-none m-b-none">
                        {{$tk_khoahoc->appends(Request::all())->links() }}
                    </ul>
                </div>
            </div>
        </div>
